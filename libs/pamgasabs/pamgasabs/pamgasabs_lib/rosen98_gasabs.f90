...
!f2py threadsafe
   ...
   real(kind=dbl), dimension(nLayer), intent(in) :: freq ! frequency [GHz]
   real(kind=dbl), dimension(nLayer), intent(in) :: tempK ! temperature [K]
   real(kind=dbl), dimension(nLayer), intent(in) :: pres ! pressure [Pa]
   real(kind=dbl), dimension(nLayer), intent(in) :: rhoWv ! water vapor density [kg/m**3]

! Scalar arguments with intent(out):

   real(kind=dbl), dimension(nLayer), intent(out) :: absAir ! extinction by dry air [Np/km]
   real(kind=dbl), dimension(nLayer), intent(out) :: absWv ! extinction by water vapor [Np/km]
...
! Error handling
   ...
   if (verbose >= 5) print *, freq, tempK, pres, rhoWv
